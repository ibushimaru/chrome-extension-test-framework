...
class PerformanceAnalyzer {
    constructor(options = {}) {
        this.options = {
            memoryLeakThreshold: options.memoryLeakThreshold || 5,
            heavyComputationThreshold: options.heavyComputationThreshold || 1000,
            domElementThreshold: options.domElementThreshold || 1000,
            bundleSizeThreshold: options.bundleSizeThreshold || 500 * 1024, // 500KB
            ...options
        };
    ...
    detectHeavyComputation(content, fileName) {
        const patterns = [
            ...
            {
                pattern: /for\s*\([^)]+\)[\s\S]*?for\s*\([^)]+\)[\s\S]*?for\s*\([^)]+\)/g,
