...
class ContextAwareDetector {
    constructor() {
        ...
        this.safePatterns = {
            innerHTML: [
                ...
                // フレームワーク固有の安全なパターン
                /dangerouslySetInnerHTML\s*=\s*{\s*{\s*__html:/,  // React
                /v-html\s*=/,  // Vue
                /\[innerHTML\]\s*=/,  // Angular
                /sanitize.*innerHTML/i,
                /createSanitizer/
            ],
            localStorage: [
                // 開発環境チェック
    ...
    getLineNumber(content, position) {
        ...
        if (position >= content.length) return content.split('\n').length;
        
        // More accurate: count actual newlines up to position
        const lines = content.substring(0, position + 1).split('\n');
        return lines.length;
    }
    ...
    /**
     * フレームワーク固有の安全なパターンかチェック
     */
    isFrameworkSafePattern(content, position, filePath = '') {
        const frameworks = this.detectFramework(content, filePath);
        
        // フレームワークが検出されなくても、安全なパターンをチェック
        const context = content.substring(Math.max(0, position - 200), position + 200);
        
        // React の安全なパターン
        if (/dangerouslySetInnerHTML\s*=\s*{\s*{\s*__html:/.test(context)) {
            return true;
        }
        
        // Vue の安全なパターン
        if (/v-html\s*=/.test(context)) {
            return true;
        }
        
        // Angular の安全なパターン
        if (/\[innerHTML\]\s*=/.test(context)) {
            return true;
        }
        
        // サニタイザーの使用
        if (/sanitize|DOMPurify|dompurify|xss/i.test(context)) {
            return true;
        }
    ...
    detectUnsafeInnerHTML(content, filePath) {
        ...
        while ((match = regex.exec(content)) !== null) {
            ...
            // フレームワーク固有の安全なパターンかチェック
            if (this.isFrameworkSafePattern(content, position, filePath)) {
                continue;
            }
    ...
    isSafeInnerHTMLAssignment(assignedValue, content) {
        ...
        // 定数文字列（HTMLタグを含まない、かつテンプレートリテラルでない）
        if (/^['"`][^'"`<>]*['"`]$/.test(assignedValue)) {
            // テンプレートリテラルの場合は安全ではない
            if (assignedValue.startsWith('`') && assignedValue.includes('${')) {
                return false;
            }
            return true;
        }
