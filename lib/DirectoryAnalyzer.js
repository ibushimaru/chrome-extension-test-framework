    ...
    async buildStructure(currentPath, structure, relativePath, depth) {
        try {
            ...
            for (const entry of entries) {
                ...
                // ExcludeManagerがある場合はそれを使用
                if (this.excludeManager) {
                    if (entry.isDirectory() && this.excludeManager.shouldExcludeDirectory(entryPath)) {
                        continue;
                    }
                    if (!entry.isDirectory() && this.excludeManager.shouldExclude(entryPath)) {
                        continue;
                    }
                } else {
                    // フォールバック: 元の除外ロジック
