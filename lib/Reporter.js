            ...
            generate: (results) => {
                ...
                if (isQuiet) {
                    if (summary.failed > 0 || (results.warnings && results.warnings.length > 0)) {
                        // Summary line for CI
                        console.log(`cext-test: ${summary.failed} errors, ${results.warnings?.length || 0} warnings`);
                        
                        // Errors
                        if (summary.errors && summary.errors.length > 0) {
                            summary.errors.forEach(error => {
                                console.error(`ERROR: [${error.suite}] ${error.test} - ${error.message}`);
                            });
                        }
                ...
                if (summary.errors && summary.errors.length > 0) {
                    ...
                    summary.errors.forEach(error => {
                        ...
                        if (error.error.enhanced) {
                            ...
                            console.log(`   Level: ${enhanced.level}`);
