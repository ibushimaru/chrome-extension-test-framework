                ...
                if (summary.errors.length > 0) {
                    ...
                    summary.errors.forEach(error => {
                        ...
                        if (error.error.formattedMessage) {
                            console.log(error.error.formattedMessage.split('\n').map(line => `   ${line}`).join('\n'));
                        } else {
        ...
        <div class="suite">
            ...
            <div class="suite-body">
                ${suite.tests.map(test => `
                    ...
                    ${test.error ? `<div class="error">${this.escapeHtml(test.error.formattedMessage || test.error.message)}</div>` : ''}
                `).join('')}
            </div>
        </div>
    `).join('')}
    ...
    generateMarkdown(results) {
        ...
        results.suites.forEach(suite => {
            ...
            suite.tests.forEach(test => {
                ...
                if (test.error) {
                    if (test.error.formattedMessage) {
                        // Format the enhanced error message for markdown
                        const errorLines = test.error.formattedMessage.split('\n');
