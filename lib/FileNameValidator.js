...
class FileNameValidator {
    constructor(options = {}) {
        ...
        // ExcludeManagerを使用
        this.excludeManager = options.excludeManager || null;
    ...
    async validateDirectory(dirPath, extensionPath = '') {
        ...
        const excludeManager = this.excludeManager;
        ...
        async function scanDir(currentPath, basePath) {
            ...
            for (const entry of entries) {
                ...
                // ExcludeManagerがある場合は除外チェック
                if (excludeManager) {
                    if (entry.isDirectory() && excludeManager.shouldExcludeDirectory(fullPath)) {
                        continue;
                    }
                    if (!entry.isDirectory() && excludeManager.shouldExclude(fullPath)) {
                        continue;
                    }
                }
                ...
                if (entry.isDirectory()) {
                    // ディレクトリ名も検証
                    const validator = new FileNameValidator(options);
