...
class SecurityAnalyzer {
    constructor(options = {}) {
        ...
        this.severityManager = new SeverityManager(options);
        this.errorMessages = new ImprovedErrorMessages();
        this.excludeManager = new ExcludeManager(options);
        
        // 診断モード（環境変数またはconfigで有効化）
        this.diagnosticMode = process.env.CEXT_DIAGNOSTIC === 'true' || options.diagnostic === true;
     ...
     * 拡張機能全体を分析
     */
    async analyze(extensionPath, excludeManager = null) {
        // ExcludeManagerを設定（引数で渡されるか、コンストラクタで設定されたもの）
        if (excludeManager) {
        ...
        // 拡張機能のベースパスを保存
        this.extensionBasePath = path.resolve(extensionPath);
    ...
    async scanDirectory(dir, basePath = '') {
        // ディレクトリが拡張機能のベースパス内にあるか確認
        const resolvedDir = path.resolve(dir);
        if (!resolvedDir.startsWith(this.extensionBasePath)) {
            // 拡張機能外のディレクトリはスキップ
            return;
        }
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                // ExcludeManagerでチェック
                if (!this.excludeManager.shouldExcludeDirectory(relativePath)) {
                    await this.scanDirectory(fullPath, relativePath);
                }
            } else if (entry.isFile()) {
                // ExcludeManagerでチェック
                if (!this.excludeManager.shouldExclude(relativePath)) {
    ...
    async scanFile(filePath, relativePath) {
        // ファイルが拡張機能のベースパス内にあるか確認
        const resolvedPath = path.resolve(filePath);
        if (!resolvedPath.startsWith(this.extensionBasePath)) {
            // 拡張機能外のファイルはスキップ
            return;
        }
    ...
    checkPatterns(content, filePath, patterns) {
        ...
        patterns.forEach(patternDef => {
            ...
            for (const match of matches) {
                ...
                // 診断モードでファイル情報を出力
                if (this.diagnosticMode && lineNumber > lines.length) {
                    console.warn(`\n⚠️  Line number mismatch detected:`);
                    console.warn(`   File: ${filePath}`);
                    console.warn(`   Calculated line: ${lineNumber}`);
                    console.warn(`   Total lines in file: ${lines.length}`);
                    console.warn(`   Pattern: ${patternDef.name}`);
                    DiagnosticHelper.printDiagnostics(filePath, lineNumber);
                }
    ...
    getLineNumber(content, index) {
        ...
        if (index >= content.length) return content.split('\n').length;
        
        // More accurate: count actual newlines up to position
        const lines = content.substring(0, index + 1).split('\n');
        return lines.length;
    }
